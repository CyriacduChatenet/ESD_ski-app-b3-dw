...
export class BookingService implements Booking {
	...
	async updateOne(url: string, credentials: Object) {
		return await this.useFetch.patchProtected(url, credentials, String(localStorage.getItem('acessToken')));
	}
	...
	async createAndUpdate(booking_url: string, post_url: string, credentials: Object) {
		...
		const post = await this.postService.updateOne(post_url, { bookings: [`${booking._id}`], isAvailable: false});
	};
